...
//#endif // FULLSCREEN
	...
	int timeLoc = ((PFNGLGETUNIFORMLOCATIONPROC)wglGetProcAddress("glGetUniformLocation"))(p, "y");
	PFNGLUNIFORM1FPROC oglUniform1f = (PFNGLUNIFORM1FPROC)wglGetProcAddress("glUniform1f");
    ...
    do
    { 
		// Not works for large period of time
		//glColor3us((unsigned short)(10*(float)(GetTickCount() & 65535)), 0, 0);

		QueryPerformanceCounter(&li);
		float time = float((li.QuadPart - CounterStart) / PCFreq);
		oglUniform1f(timeLoc, time / 1000.f);
